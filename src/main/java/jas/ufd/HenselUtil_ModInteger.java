...
public class HenselUtil_ModInteger {
    ...
    public static List<GenPolynomial<ModInteger>> liftHenselMonic(GenPolynomial<JasBigInteger> C, List<GenPolynomial<ModInteger>> F, long k) {
        ...
        if (n == 1) { // lift F_0, this case will probably never be used
            ...
            if (ModLongRing.MAX_LONG.compareTo(P.getVal()) > 0) {
                ...
                mcfac = new ModIntegerRing(P.getVal());
            }
            GenPolynomialRing<ModInteger> mfac = new GenPolynomialRing<>(mcfac, fac);
            f = PolyUtil.fromIntegerCoefficients(mfac, PolyUtil_ModInteger.integerFromModularCoefficients(fac, f));
            lift.add(f);
            return lift;
        }

        GenPolynomialRing<JasBigInteger> ifac = new GenPolynomialRing<>(new JasBigInteger(), fac);
        List<GenPolynomial<JasBigInteger>> Fi = PolyUtil_ModInteger.integerFromModularCoefficients(ifac, F);

        List<GenPolynomial<ModInteger>> S = liftExtendedEuclidean(F, k + 1); // lift works for any k, TODO: use this
        // adjust coefficients
        List<GenPolynomial<ModInteger>> Sx = PolyUtil.fromIntegerCoefficients(pfac, PolyUtil_ModInteger
        ...
        List<GenPolynomial<JasBigInteger>> Si = PolyUtil_ModInteger.integerFromModularCoefficients(ifac, S);
        ...
        if (ModLongRing.MAX_LONG.compareTo(modul.getVal()) > 0) {
            ...
            mcfac = new ModIntegerRing(modul.getVal());
        }
