...
public strictfp abstract class QBarIterableProvider {
    ...
    /**
     * Generates {@code AlgebraicAngle}s that are rational multiples of π between {@code a} and {@code b}, inclusive.
     * If {@code a}{@literal <}{@code b}, the angles produced will be greater than or equal to {@code a} and less than
     * or equal to {@code b}. If {@code a}{@literal >}{@code b}, the range wraps around zero, so the angles produced
     * are greater than or equal to {@code a} <i>or</i> less than or equal to {@code b}. If {@code a} and {@code b} are
     * equal and a rational multiple of π, the result can only produce that angle. If they are equal and not a rational
     * multiple of π, no angle can be produced (and this may be an error, depending on the provider type).
     *
     * <ul>
     *  <li>{@code a} cannot be null.</li>
     *  <li>{@code b} cannot be null.</li>
     ...
     */
    public abstract @NotNull Iterable<AlgebraicAngle> rationalMultiplesOfPiInRange(
